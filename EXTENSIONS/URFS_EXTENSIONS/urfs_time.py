...
def reverse_subsystem_dynamics(R_t, X, W_f, W_b, scale_factor=1.0, epsilon=1e-8):
    ...
    return (W_b * R_t - W_f * X) / (W_b + epsilon) #Add an epsilon to denominator
...
# --- Main Loop (Forward Pass) ---
R_history = [X_t] # store states as the results
for t in range(n_iterations):
    ...
    weights = [
        weight_functions[i](variances[i], smoothing=0.95, prev_weights=weights[i])
        for i in range(n_subsystems)
    ]


    R_history.append(X_t_next)
    X_t = X_t_next
...
for t in range(n_iterations):
    ...
    if t_reverse < 1:
      break # stop if we are at t=0

    X_t_reverse_next = [
        reverse_subsystem_dynamics(X_t_reverse[i], R_history[t_reverse-1][i] if i != 2 else R_history[t_reverse-1][i], weights[i], weights[i])
            for i in range(n_subsystems)
        ]

    V_t = compute_lyapunov(X_t_reverse, X_star, weights)
